...
def pelanggan_dashboard():
    if 'role' in session and session['role'] == 'pelanggan':
        # Ambil data pesanan terbaru untuk pelanggan
        cur = mysql.connection.cursor()
        cur.execute("""
            SELECT p.id, m.nama, p.jumlah, p.status, p.created_at
            FROM pesanan p
            JOIN menu m ON p.id_menu = m.id
            WHERE p.id_user = %s
            ORDER BY p.created_at DESC
            LIMIT 5
        """, (session['user_id'],))
        pesanan_terakhir = cur.fetchall()
        cur.close()
        
        return render_template('pelanggan/dashboard.html', 
                            current_time=datetime.now(),
                            pesanan_terakhir=pesanan_terakhir)
    else:
